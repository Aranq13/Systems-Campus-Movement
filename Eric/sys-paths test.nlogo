...
to setup
  ...
  import-pcolors "mapbox trinty 3.png"
...
to check-building-placement
  ifelse mouse-down? [
    if not mouse-clicked? [
     ...
     if placement_of_object = "building"[
        ask patch mouse-xcor mouse-ycor [ toggle-building ]]
     if placement_of_object = "junction"[
        ask patch mouse-xcor mouse-ycor [ toggle-junction ]]
    ]
  ] [
...
to toggle-junction
  let nearby-junctions junctions in-radius 4
  ifelse any? nearby-junctions [
    ; if there is a building near where the mouse was clicked
    ; (and there should always only be one), we remove it and
    ask nearby-junctions [ die ]
  ] [
    ; if there was no buildings near where
    ; the mouse was clicked, we create one
    sprout-junctions 1 [
...
to become-more-popular
  ...
  if popularity >= maximum-popularity [set pcolor black]
end
...
to move-walkers
  ask walkers [
    ...
    ; ifelse [pcolor] of patch-ahead 1 = 126
    ;[ rt random-normal 0 45 ]
   ; [ fd random 2 ]
      ]
    ] [
...
to walk-towards-goal
 ; if pcolor != gray and pcolor != 126
  ...
  ; this is the vibrational thingy
   ifelse [pcolor] of patch-ahead 5 = 126 ;or [pcolor] of patch-ahead 1 = black
    [ rt random-normal 90 180
      if[pcolor] of patch-ahead 5 != 126
      [fd 5 ]
      ]
    [ fd 1 ]
end
...
to-report best-way-to [ destination ]
  ...
  let visible-patches patches in-radius walker-vision-dist ;and patches with [pcolor != 126];
  ...
  let worse-routes visible-patches with [ pcolor = black ]
  let worse-routes-that-take-me-closer worse-routes with [
    distance destination < [ distance destination - 1 ] of myself
  ]
  ...
  ] [ifelse any? worse-routes-that-take-me-closer [
    ; from those route patches, choose the one that is the closest to me
      report min-one-of worse-routes-that-take-me-closer [ distance self ]]
...
to recolor-patches
  ...
  ifelse show-popularity? [
    ...
    ask patches with [ pcolor != gray  and pcolor != 126 and pcolor != black] [
